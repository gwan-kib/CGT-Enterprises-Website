...
- Use Google Material Symbols (Rounded variant only) for interface symbols. Always apply the `material-symbols-rounded` class. Load them once through a central document or stylesheet entrypoint instead of maintaining local icon files or adding an icon package.
