...
- Clearly labelled placeholder copy, pricing, contact details, and image areas.
- Customer reviews loaded on page visits from the shared Apps Script public feed, with inline loading, empty, and unavailable states and the existing draggable carousel.
...
`VITE_APPS_SCRIPT_URL` configures the shared Google Apps Script web app for contact/review POST submissions and public review GET requests. It is public configuration, not a secret; see `.env.example` for the placeholder. Until it is set, the forms report a submission error. Local `.env` files are excluded from version control. Restart Vite after changing the value locally. Set the same base `/exec` URL in the Render frontend environment and rebuild/redeploy after changing it because Vite embeds this configuration at build time. Do not append `resource=reviews` to the configured base URL; the feed utility adds it only to GET requests.
